...
mixin PageMixin<T extends StatefulWidget> on State<T> {
  ...
  void _animateScale(double from, double to) {
    _scaleAnimation = Tween<double>(begin: from, end: to)
        .animate(_scaleAnimationController);
    _scaleAnimationController
      ..value = 0.0
      ..fling(velocity: 0.4);  // 使用 fling 让动画更自然
  }
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    // 如果当前缩放比例 < 1.0，则自动恢复到 1.0
    if (mScale < 1.0) {
      _animateScale(mScale, 1.0);
      _animatePosition(mOffset, Offset.zero);
      _scaleAnimationController.forward();
      _positionAnimationController.forward();
    }
    setState(() {});
